...
class Mesh():
    ...
    def plotmesh(self, **how):
        ...
        """
        hx = 0.05*(self.Mx-self.mx)
        hy = 0.05*(self.My- self.my)
        ax = plt.gca()
        ...
        for k in range(self.nvol):
            plt.text(self.centres[k, 0]+hx,
                     self.centres[k, 1]+hy,
                     str(k))
...
def plotdiscrete(E, f, **data):
    ...
    def name_legd(val):
        ...
        q = np.argsort(val)
        del leg.texts[:]
        for i in range(legd+1):
            leg.text(wl+0.1, i*hl-0.01,
                     "{0:.2e}".format(val[q[int(i*len(val)/(legd+1))]]))
    ...
    if 'scale' not in data.keys() or data['scale'] == 'free':
        ...
        if 'warning' in data.keys() and data['warning'] == 'on':
            if vinf > m:
                wng.warn('Plot scale cannot handle low values of the function')
            if vsup < M:
                wng.warn('Plot scale cannot handle' +
                         'high values of the function')
        val[val < vinf] = vinf
        val[val > vsup] = vsup
        nf = (val-vinf)/(vsup-vinf)

    if not fig.axes:
        mplot = fig.add_axes([0.125, 0.1, 0.8, height])
        ...
        mplot.set_anchor('W')
        leg = fig.add_axes([0.8, 0.1, 0.1, height])
        leg.set(ylim=(0, 1), xlim=(0, 1))
        leg.set_axis_off()
        leg.set_anchor('W')
        for i in range(100):
            plt.fill([0, 0.25, 0.25, 0], [i/100, i/100, (i+1)/100, (i+1)/100],
                     color=paint(i/100))
        if 'scale' in data.keys() and data['scale'] != 'free':
            name_legd(val)
    else:
        mplot = fig.axes[-2]
        leg = fig.axes[-1]
...
def surf(E, f, ax=None, **kwargs):
    if ax is None:
        ax=plt.gca()
    ax.plot_trisurf(E.centres[:, 0], E.centres[:, 1], f, kwargs)
